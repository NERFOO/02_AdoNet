...
ALTER PROCEDURE [dbo].[SP_EMPLEADOS_HOSPITAL] (@NOM NVARCHAR(50), @SUMA INT OUT, @MEDIA INT OUT, @PERSONAS INT OUT)
AS
	DECLARE @ID INT
	SELECT @ID = HOSPITAL_COD FROM HOSPITAL WHERE NOMBRE = @NOM

	SELECT HOSPITAL_COD, APELLIDO, SALARIO FROM PLANTILLA WHERE HOSPITAL_COD = @ID
	UNION
	SELECT HOSPITAL_COD, APELLIDO, SALARIO FROM DOCTOR WHERE HOSPITAL_COD = @ID

	SELECT @SUMA = SUM(SALARIO), @MEDIA = AVG(SALARIO), @PERSONAS = COUNT(EMPLEADOS) 
		FROM (
    		SELECT APELLIDO, SALARIO, DOCTOR_NO AS EMPLEADOS FROM DOCTOR WHERE HOSPITAL_COD = @ID 
			UNION
    		SELECT APELLIDO, SALARIO, EMPLEADO_NO FROM PLANTILLA WHERE HOSPITAL_COD = @ID   
   		) QUERY
GO
 */
#endregion
